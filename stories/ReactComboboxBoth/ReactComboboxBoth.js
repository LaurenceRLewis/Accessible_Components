  ...
  const onKeydown = (event) => {
    switch (event.key) {
      ...
      case 'Delete':
        if (userInput !== '') {
          event.preventDefault();
          const newValue = userInput.slice(1);
          setUserInput(newValue);
          setPredictiveText(newValue);
          const newFilteredOptions = filterOptions(newValue);
          setFilteredOptions(newFilteredOptions);
  ...
  return (
    <div className={styles["comboboxContainer"]}>
      ...
      <input
        ...
        ref={inputRef}
      />
        <button id={styles["toggle-button"]} aria-label="Show options" tabIndex="-1" onClick={onToggleButtonClick}>
      ...
      >
        {filteredOptions.map((option, index) => 
          <li role="option" aria-selected={selectedIndex === index ? "true" : "false"} onClick={() => selectOption(index)} key={index} className={selectedIndex === index ? styles["selectedOption"] : ''}>
            {option}
          </li>)}
      </ul>
    </div>
