...
`),installableProjectTypes.sort().forEach(framework=>paddedLog(`- ${framework}`)),logger6.log(),new HandledError(`Unknown project type supplied: ${projectTypeProvided}`);else try{projectType=detect(packageJson,options)}catch(err){throw done(err.message),new HandledError(err)}if(done(),isStorybookInstantiated()&&projectType!=="ANGULAR"){logger6.log();let{force}=await(0,import_prompts5.default)([{type:"confirm",name:"force",message:"We found a .storybook config directory in your project. Therefore we assume that Storybook is already instantiated for your project. Do you still want to continue and force the initialization?"}]);logger6.log(),force?options.force=!0:process.exit(0)}let installResult=await installStorybook(projectType,packageManager,options);options.skipInstall||await packageManager.installDependencies(),options.disableTelemetry||(0,import_telemetry.telemetry)("init",{projectType}),logger6.log(`
For more information visit:`,import_chalk4.default.cyan("https://storybook.js.org")),projectType==="ANGULAR"?(logger6.log(`
To run your Storybook, type:
`),codeLog([`ng run ${installResult.projectName}:storybook`])):projectType==="REACT_NATIVE"?(logger6.log(),logger6.log(import_chalk4.default.yellow(`NOTE: installation is not 100% automated.
`)),logger6.log(`To quickly run Storybook, replace contents of your app entry with:
`),codeLog(["export {default} from './.storybook';"]),logger6.log(`
 Then to run your Storybook, type:
`),codeLog([packageManager.getRunCommand("start")]),logger6.log(`
 For more in information, see the github readme:
`),logger6.log(import_chalk4.default.cyan("https://github.com/storybookjs/react-native")),logger6.log()):(logger6.log(`
To run your Storybook, type:
`),codeLog([packageManager.getRunStorybookCommand()])),logger6.log()}async function initiate(options,pkg2){await(0,import_core_server.withTelemetry)("init",{cliOptions:options,printError:err=>!err.handled&&logger6.error(err)},()=>doInitiate(options,pkg2))}var import_path10=__toESM(require("path")),import_fs6=__toESM(require("fs")),import_cross_spawn2=require("cross-spawn"),import_core_common=require("@storybook/core-common"),import_csf_tools2=require("@storybook/csf-tools");var logger7=console,LEGACY_CONFIGS=["addons","config","presets"],postinstallAddon=async(addonName,isOfficialAddon)=>{let skipMsg=null;isOfficialAddon?import_fs6.default.existsSync(".storybook")?(skipMsg="no codmods found",LEGACY_CONFIGS.forEach(config=>{try{let codemod=require.resolve(`${getPackageName(addonName,isOfficialAddon)}/postinstall/${config}.js`);commandLog(`Running postinstall script for ${addonName}`)();let configFile=import_path10.default.join(".storybook",`${config}.ts`);import_fs6.default.existsSync(configFile)||(configFile=import_path10.default.join(".storybook",`${config}.js`),import_fs6.default.existsSync(configFile)||import_fs6.default.writeFileSync(configFile,"","utf8")),(0,import_cross_spawn2.sync)("npx",["jscodeshift","-t",codemod,configFile],{stdio:"inherit",shell:!0}),skipMsg=null}catch{}})):skipMsg="no .storybook config":skipMsg="unofficial addon",skipMsg&&commandLog(`Skipping postinstall for ${addonName}, ${skipMsg}`)()},getVersionSpecifier=addon=>{let groups=/^(...*)@(.*)$/.exec(addon);return groups?[groups[1],groups[2]]:[addon,void 0]};async function add(addon,options){let{packageManager:pkgMgr}=options;options.useNpm&&(useNpmWarning(),pkgMgr="npm");let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),packageJson=await packageManager.retrievePackageJson(),[addonName,versionSpecifier]=getVersionSpecifier(addon),{mainConfig,version:storybookVersion}=(0,import_core_common.getStorybookInfo)(packageJson);if(!mainConfig){logger7.error("Unable to find storybook main.js config");return}let main=await(0,import_csf_tools2.readConfig)(mainConfig);logger7.log(`Verifying ${addonName}`);let latestVersion=await packageManager.latestVersion(addonName);latestVersion||logger7.error(`Unknown addon ${addonName}`);let isStorybookAddon=addonName.startsWith("@storybook/"),addonWithVersion=`${addonName}@${versionSpecifier||(isStorybookAddon?storybookVersion:latestVersion)}`;logger7.log(`Installing ${addonWithVersion}`),await packageManager.addDependencies({installAsDevDependencies:!0},[addonWithVersion]),logger7.log(`Adding '${addon}' to main.js addons field.`),main.appendValueToArray(["addons"],addonName),await(0,import_csf_tools2.writeConfig)(main),options.skipPostinstall||await postinstallAddon(addon,isStorybookAddon)}var import_codemod=require("@storybook/codemod");var import_prompts8=__toESM(require("prompts")),import_chalk26=__toESM(require("chalk")),import_boxen2=__toESM(require("boxen")),import_fs_extra11=require("fs-extra"),import_tempy=__toESM(require("tempy")),import_ts_dedent30=__toESM(require("ts-dedent")),import_path14=require("path"),import_core_common8=require("@storybook/core-common"),import_semver23=__toESM(require("semver"));var import_chalk8=__toESM(require("chalk")),import_ts_dedent11=require("ts-dedent"),import_semver10=__toESM(require("semver"));var import_chalk7=__toESM(require("chalk")),import_ts_dedent10=require("ts-dedent"),import_semver9=__toESM(require("semver"));var import_chalk6=__toESM(require("chalk")),import_semver8=__toESM(require("semver")),import_ts_dedent9=__toESM(require("ts-dedent"));var import_core_common2=require("@storybook/core-common"),import_csf_tools3=require("@storybook/csf-tools"),import_chalk5=__toESM(require("chalk")),import_semver7=__toESM(require("semver")),import_ts_dedent8=__toESM(require("ts-dedent")),logger8=console,getStorybookData=async({packageManager,configDir:userDefinedConfigDir})=>{var _a;let packageJson=await packageManager.retrievePackageJson(),{mainConfig:mainConfigPath,version:storybookVersionSpecifier,configDir:configDirFromScript,previewConfig:previewConfigPath}=(0,import_core_common2.getStorybookInfo)(packageJson,userDefinedConfigDir),storybookVersion=storybookVersionSpecifier&&((_a=import_semver7.default.coerce(storybookVersionSpecifier))==null?void 0:_a.version),configDir=userDefinedConfigDir||configDirFromScript||".storybook",mainConfig;try{mainConfig=await(0,import_core_common2.loadMainConfig)({configDir,noCache:!0})}catch(err){throw new Error(import_ts_dedent8.default`Unable to find or evaluate ${import_chalk5.default.blue(mainConfigPath)}: ${err.message}`)}return{configDir,mainConfig,storybookVersionSpecifier,storybookVersion,mainConfigPath,previewConfigPath}},updateMainConfig=async({mainConfigPath,dryRun},callback)=>{try{let main=await(0,import_csf_tools3.readConfig)(mainConfigPath);await callback(main),dryRun||await(0,import_csf_tools3.writeConfig)(main)}catch(e){logger8.info(`\u274C The migration failed to update your ${import_chalk5.default.blue(mainConfigPath)} on your behalf because of the following error:
        ${e}
`),logger8.info(`\u26A0\uFE0F Storybook automigrations are based on AST parsing and it's possible that your ${import_chalk5.default.blue(mainConfigPath)} file contains a non-standard format (e.g. your export is not an object) or that there was an error when parsing dynamic values (e.g. "require" calls, or usage of environment variables). When your main config is non-standard, automigrations are unfortunately not possible. Please follow the instructions given previously and follow the documentation to make the updates manually.`)}},getAddonNames=mainConfig=>(mainConfig.addons||[]).map(addon=>{let name="";if(typeof addon=="string"?name=addon:typeof addon=="object"&&(name=addon.name),!name.startsWith("."))return name.replace(/\/dist\/.*/,"").replace(/\.[mc]?[tj]?s[x]?$/,"").replace(/\/register$/,"").replace(/\/manager$/,"").replace(/\/preset$/,"")}).filter(Boolean);var logger9=console,checkWebpack5Builder=async({configDir,packageManager})=>{var _a;let{mainConfig,storybookVersion}=await getStorybookData({configDir,packageManager});if(import_semver8.default.lt(storybookVersion,"6.3.0"))return logger9.warn(import_ts_dedent9.default`
      ...
      `.trim()),null;if(import_semver8.default.gte(storybookVersion,"7.0.0"))return null;if(!mainConfig)return logger9.warn("Unable to find storybook main.js config"),null;let builder=(_a=mainConfig.core)==null?void 0:_a.builder;return builder&&builder!=="webpack4"?(logger9.info(`Found builder ${builder}, skipping`),null):{storybookVersion}};var logger10=console,webpack5={id:"webpack5",async check({configDir,packageManager}){var _a;let webpackVersion=(await packageManager.retrievePackageJson()).dependencies.webpack,webpackCoerced=(_a=import_semver9.default.coerce(webpackVersion))==null?void 0:_a.version;if(!webpackCoerced||import_semver9.default.lt(webpackCoerced,"5.0.0")||import_semver9.default.gte(webpackCoerced,"6.0.0"))return null;let builderInfo=await checkWebpack5Builder({configDir,packageManager});return builderInfo?{webpackVersion,...builderInfo}:null},prompt({webpackVersion}){let webpackFormatted=import_chalk7.default.cyan(`webpack ${webpackVersion}`);return import_ts_dedent10.dedent`
...
`)}
    ...
    `}};var import_chalk17=__toESM(require("chalk")),import_ts_dedent21=require("ts-dedent"),import_path12=require("path"),import_fs_extra10=__toESM(require("fs-extra")),import_globby2=__toESM(require("globby")),MDX1_STYLE_START=/<style>{`/g,MDX1_STYLE_END=/`}<\/style>/g,MDX1_COMMENT=/<!--(.+)-->/g,MDX1_CODEBLOCK=/(?:\n~~~(?:\n|.)*?\n~~~)|(?:\n```(?:\n|.)*?\n```)/g,fixMdxStyleTags=mdx=>mdx.replace(MDX1_STYLE_START,"<style>\n  {`").replace(MDX1_STYLE_END,"  `}\n</style>"),fixMdxComments=mdx=>{let codeblocks=mdx.matchAll(MDX1_CODEBLOCK);return mdx.split(MDX1_CODEBLOCK).map(v=>v.replace(MDX1_COMMENT,(original,group)=>`{/*${group}*/}`)).reduce((acc,item,i)=>{let next=codeblocks.next();return next.done?acc+item:acc+item+next.value[0]},"")},logger18=console,mdx1to2={id:"mdx1to2",async check(){let storiesMdxFiles=await(0,import_globby2.default)("./!(node_modules)**/*.(story|stories).mdx");return storiesMdxFiles.length?{storiesMdxFiles}:void 0},prompt({storiesMdxFiles}){return import_ts_dedent21.dedent`
    ...
    `},async run({result:{storiesMdxFiles},dryRun}){await Promise.all([...storiesMdxFiles.map(async fname=>{let contents=await import_fs_extra10.default.readFile(fname,"utf-8"),updated=fixMdxComments(fixMdxStyleTags(contents));updated===contents?logger18.info(`\u{1F197} Unmodified ${(0,import_path12.basename)(fname)}`):(logger18.info(`\u2705 Modified ${(0,import_path12.basename)(fname)}`),dryRun||await import_fs_extra10.default.writeFile(fname,updated))})])}};var import_chalk18=__toESM(require("chalk")),import_ts_dedent22=require("ts-dedent");var logger19=console,autodocsTrue={id:"autodocsTrue",async check({packageManager,configDir}){let{mainConfig}=await getStorybookData({packageManager,configDir}),{docs}=mainConfig,docsPageToAutodocsMapping={true:"tag",automatic:!0,false:!1};if(docs!=null&&docs.docsPage){let oldValue=docs==null?void 0:docs.docsPage.toString();if(!(oldValue in docsPageToAutodocsMapping))throw new Error(`Unexpected value for docs.docsPage: ${oldValue}`);return{value:docsPageToAutodocsMapping[oldValue]}}return(docs==null?void 0:docs.autodocs)===void 0?{value:!0}:null},prompt({value}){let autodocsFormatted=import_chalk18.default.cyan(`docs: { autodocs: ${JSON.stringify(value??!0)} }`),tagWarning=import_ts_dedent22.dedent`
...
`);return import_ts_dedent23.dedent`
    ...
    `},async run(){logger20.info(),await generateStorybookBabelConfigInCWD()}};var import_ts_dedent26=require("ts-dedent"),import_semver19=__toESM(require("semver")),import_chalk22=__toESM(require("chalk")),import_prompts7=__toESM(require("prompts"));var angularBuilders={id:"angular-builders",async check({packageManager,configDir}){var _a;let packageJSON=await packageManager.retrievePackageJson();if(isNxProject(packageJSON))return null;let angularVersion=(await packageManager.getAllDependencies())["@angular/core"],angularCoerced=(_a=import_semver19.default.coerce(angularVersion))==null?void 0:_a.version;if(!angularCoerced)return null;if(import_semver19.default.lt(angularCoerced,"14.0.0"))throw new Error(import_ts_dedent26.dedent`
      ...
      `);let angularJSON=new AngularJSON,{hasStorybookBuilder}=angularJSON;if(hasStorybookBuilder||!angularJSON.rootProject&&Object.keys(angularJSON.projects).length>1)return null;let{mainConfig}=await getStorybookData({configDir,packageManager});return{mainConfig,packageManager}},prompt(){return import_ts_dedent26.dedent`
...
`);return import_ts_dedent31.default`
    ...
    To learn more about this change, see: ${import_chalk27.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
  `},async run({dryRun,mainConfigPath,result:{nextStoriesEntries}}){logger22.info(import_ts_dedent31.default`✅ Setting 'stories' config:
...
`)}
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),process.exit(1)),choices.length===1?[selectedTemplate]=choices:(logger26.info((0,import_boxen3.default)(import_ts_dedent32.dedent`
...
yarn storybook`):`Recreate your setup, then ${import_chalk28.default.yellow("npx storybook@latest init")}`;logger26.info((0,import_boxen3.default)(import_ts_dedent32.dedent`
